...
from utils import save_model, load_model, neutralize, get_biggest_change_features, validation_metrics, \
    download_data, load_model_config, save_model_config, get_time_series_cross_val_splits, TARGET_COLUMNS, \
...
if model_selection_loop:
    ...
    download_data(numerai_api, 'numerai_training_data.parquet', 'training_data.parquet')
    ...
    for split, train_test_split in enumerate(train_test_zip):
        ...
        for target in targets:
            ...
            if not split_model:
                ...
                split_model = LGBMRegressor(**TARGET_OPTIMUM_PARAMS[target])
    ...
    training_stats = validation_metrics(training_data, all_model_cols, TARGET_COL)
    ...
    for target in targets:
        ...
        if not model:
            ...
            model = LGBMRegressor(**TARGET_OPTIMUM_PARAMS[target])
...
download_data(numerai_api, 'numerai_tournament_data.parquet', f'tournament_data_{current_round}.parquet')
print("downloading validation_data")
download_data(numerai_api, 'numerai_validation_data.parquet', 'validation_data.parquet')
...
validation_stats = validation_metrics(validation_data, [best_pred_col], TARGET_COL)
